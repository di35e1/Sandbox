...
class AudioLevelMeter:
    def __init__(self):
        ...
        self.window_height = 350  # Увеличили высоту для кнопок
    ...
    def setup_ui(self):
        ...
        # Кнопка Monitor
        self.monitor_button_canvas = tk.Canvas(
            self.buttons_frame,
            width=self.window_width,
            height=20,
            bg='black',
        ...
        self.monitor_button_canvas.pack(pady=2)
        self.setup_button(self.monitor_button_canvas, "Monitor", self.toggle_monitor)
    ...
    def on_button_enter(self, event):
        ...
        if canvas == self.close_button_canvas:
            ...
            color = '#cc0000' if not self.recording else '#cc0000'
        elif canvas == self.record_button_canvas and self.recording:
            canvas.itemconfig(canvas.button_text, fill='red')
        else:
            color = '#0066cc' if not self.monitoring else '#cc6600'
    ...
    def on_button_leave(self, event):
        ...
        if canvas == self.monitor_button_canvas and self.monitoring:
            canvas.itemconfig(canvas.button_text, fill='orange')
        elif canvas == self.record_button_canvas and self.recording:
            canvas.itemconfig(canvas.button_text, fill='red')
        else:
    ...
    def toggle_record(self):
        ...
        if not self.recording:
            ...
            try:
                ...
                self.record_button_canvas.itemconfig(self.record_button_canvas.button_text, fill='red')
    ...
    def toggle_monitor(self):
        """Включает/выключает мониторинг звука"""
        if not self.monitoring:
            # Включаем мониторинг
            try:
                # Очищаем буфер перед началом мониторинга
                ...
                self.output_stream = sd.OutputStream(
                    ...
                    blocksize=1024,  # Используем тот же размер блока, что и у входного потока
                    callback=self.monitor_callback
                )
                self.output_stream.start()
                self.monitoring = True
                self.monitor_button_canvas.itemconfig(self.monitor_button_canvas.button_text, text="Monitor")
                self.monitor_button_canvas.itemconfig(self.monitor_button_canvas.button_text, fill='orange')
            ...
            self.monitoring = False
            self.monitor_button_canvas.itemconfig(self.monitor_button_canvas.button_text, text="Monitor")
            self.monitor_button_canvas.itemconfig(self.monitor_button_canvas.button_text, fill='#555555')
            print("Monitoring stopped")

    def monitor_callback(self, outdata, frames, time, status):
        """Callback для вывода звука в выходное устройство"""
        if status:
        ...
        with self.buffer_lock:
            if self.audio_buffer:
                ...
                if len(latest_data) >= frames:
                    # Используем нужное количество фреймов
                    outdata[:] = latest_data[:frames]
                else:
                    # Если данных недостаточно, повторяем имеющиеся данные
                    repeats = (frames + len(latest_data) - 1) // len(latest_data)
                    repeated_data = np.tile(latest_data, (repeats, 1))[:frames]
                    outdata[:] = repeated_data
            else:
                # Если буфер пуст, заполняем нулями
                outdata[:] = np.zeros((frames, self.input_channels), dtype=np.float32)
    ...
    def audio_callback(self, indata, frames, time, status):
        ...
        if self.recording and self.audio_file:
            try:
                # Конвертируем в 16-битный формат и пишем в файл
